...
> Defines the Module 14 visual-system and cross-platform usability audit: Arial/Oswald typography, Rocket Food palette, supplied wireframes, scrolling, safe areas, keyboard behavior, accessibility, and exact role tab labels. Use this document with `ai/M14/ai-spec.md` and every completed feature specification.
...
- **Specification file:** `ai/M14/features/ui.feature.md`
...
- **Visual references:** `support_materials_14/Wireframe.pdf` plus retained M13 visual contracts
...
- Compare every supplied M14 surface with `support_materials_14/Wireframe.pdf`.
...
- This feature spec and `ai/M14/ai-spec.md` when verified reality changes
...
- Claude must read the global spec, this full spec, all completed M14 specs, relevant M13 visual specs, `client/AGENTS.md`, and `client/CLAUDE.md` before edits.
- Consult exact Expo SDK 54/React Native/Expo Router accessibility, font, safe-area, list, modal, and keyboard documentation required by `client/AGENTS.md` before code changes.
...
- [ ] Claude audited before editing and changed only demonstrated gaps.
- [ ] Claude presented genuine minimum-change options and implemented only user-selected decisions.
- [ ] Palette, status colors, Arial/Oswald policy, exact tabs, and role chrome are consistent.
...
- [ ] Claude's handoff includes outcome, selected options, changed files, screen/state evidence, checks/results, manual gaps, scoped stage command, and copy-ready commit command.
