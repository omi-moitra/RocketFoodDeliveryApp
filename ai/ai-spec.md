...
1. The user's explicit current instruction and any explicit coach clarification supplied in the working conversation — immediate decision authority.
2. `ai/ai-spec.md` — committed, self-contained global implementation contract containing the reconciled grading requirements.
3. The relevant exact feature specification under `ai/features/` — executable feature contract; it may narrow this document but may not contradict it.
4. `client/docs/m14/` and `client/docs/m13/` — committed visual and project references.
5. `README.md` and `PostmanCollection.json` — committed setup, compatibility, and API evidence.
6. Existing repository code and tests — implementation reality and verification evidence, but not permission to contradict a higher-priority contract.

A repository reader must be able to follow this chain using committed files alone. Ignored planning files, private implementation notes, and removed client-root agent instruction files are not normative authorities and must not be required to understand or implement this specification.
...
For each resolved gate, Claude must update the relevant feature specification with the official requirement, verified existing-backend method, path, query, request body, response envelope, error behavior, and evidence source. Claude must then present the user with the viable minimum-change options, including a frontend-only option when one exists. For each option, state the exact contract, affected files, benefits, risks, grading/compatibility impact, and verification burden. Claude must stop and let the user choose; it must not select or implement a backend option on the user's behalf. After the user records a choice, update this file and `README.md`, add focused tests for any selected backend change, and update the frontend service boundary. Do not use this exception for cleanup, renaming, broad refactoring, speculative hardening, or unrelated API redesign.

### 1.2 Confirmed project decisions

- Continue the existing M13 repository for M14.
- Prefer frontend service adapters, but permit a user-selected minimum backend adjustment when verified frontend requirements cannot otherwise be implemented safely. Claude must present options and wait for the user's choice before editing server code. Every selected adjustment must be narrowly scoped, tested, and documented in this specification, the relevant feature spec, `README.md`, Postman when API-facing, and the private implementation log.
...
2. `ai/features/role-based-navigation.feature.md`
3. `ai/features/courier-delivery.feature.md`
4. `ai/features/account-details.feature.md`
5. `ai/features/order-confirmation-modal.feature.md`
6. `ai/features/ui.feature.md`
7. `ai/features/code-quality.feature.md`

#### Authoring status

**Feature specifications authored: 7 of 7.** The complete set is `navigation-structure.feature.md`, `role-based-navigation.feature.md`, `courier-delivery.feature.md`, `account-details.feature.md`, `order-confirmation-modal.feature.md`, `ui.feature.md`, and `code-quality.feature.md`.

`ai/features/feature-name.feature.md` is an inactive authoring scaffold, not a product feature. Delete it only at true project completion, after no feature remains to be specified or implemented.
...
The eight specifications under `ai/features/` remain regression contracts for retained Module 13 behavior unless an M14 requirement explicitly extends one. Claude must inspect the relevant M13 implementation/spec before editing a shared surface and must not weaken its acceptance criteria.
...
│       └── retained M13 feature specifications
├── client/
│   ├── app/
│   │   ├── _layout.js
│   │   ├── index.js
...
Keep the detailed M13 request, mapping, formatting, and failure rules in the retained feature specifications under `ai/features/` unless M14 explicitly changes them.
...
3. **Contract gate:** Verify required API calls in Postman or from authoritative live evidence. Record the official requirement, the existing backend's method, path, query, body, authentication, success envelope, validation, and failures in the feature spec. When they differ, present viable minimum-change options and their tradeoffs to the user, then stop before server edits.
4. **Plan:** After the user chooses an option, record that choice and map each acceptance criterion to files and verification. For a selected backend adjustment, identify the smallest affected server surface, compatibility behavior, focused tests, Postman update, frontend adapter, documentation, and rollback/recovery considerations. Stop decision-dependent work while any part remains unverified, undocumented, or unselected by the user.
...
### 20.0 Token-efficient implementation policy
...
Claude must not automatically launch an iOS Simulator, Android Emulator, physical-device session, GUI application, interactive Expo session, or full native walkthrough. It must also defer repeated full Expo exports, repeated `expo-doctor` runs, the full database-backed Maven suite, verbose dependency trees, and broad diagnostic commands unless the user explicitly requests them or a focused lower-cost check cannot resolve a concrete blocker. If a larger check becomes necessary, Claude explains why before running it.

Skipping a resource-intensive test never permits Claude to report it as passed. Deferred tests must be listed once in the final consolidated handoff with the affected feature, exact action or command, expected result, and test category.
...
- Read this file first, the relevant feature spec under `ai/features/` second, and any applicable committed repository instructions before an edit.
- Read the retained M13 feature contract under `ai/features/` when modifying established customer behavior.
...
- Present minimum-change options and tradeoffs to the user and wait for their explicit selection before making a backend adjustment; Claude must not decide on the user's behalf.
...
- **Verification:** `rg JsonAlias client server` → no matches; `./mvnw test -Dtest=OrderApiControllerTest,ApiCreateOrderDTODeserializationTest` (local MySQL) → 15 passed, 0 failures.
...
- **Verification:** `./mvnw test -Dtest=OrderApiControllerTest,ApiCreateOrderDTODeserializationTest` (local MySQL) → 15 passed, 0 failures.
...
- [x] Customer-only login routes directly to Customer.
- [x] Courier-only login routes directly to Courier.
- [x] Dual-role login routes to Account Selection and both choices are implemented.
- [x] Invalid/no-role and expired/corrupt sessions fail safely.
...
- [x] Nested Restaurant Stack and retained M13 navigation remain implemented; representative native regression is still pending.
- [x] Logout clears all identity/role data and protected route guards prevent a normal back path.
